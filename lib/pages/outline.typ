...
#let my-outline(
  ...
  show outline.entry: it => {
    ...
    let appendix-state-hide-parent = appendix-state-hide-parent.at(
      it.element.location(),
    )
    ...
    if it.level == 1 {
      ...
      if (
        counterInt.first() == 1
          and appendix-state != none
          and not appendix-state-hide-parent
      ) {
        my-outline-row(
          insetSize: 2pt,
          textWeight: "bold",
...
#let my-outline-small(
  ...
  show outline.entry: it => {
    ...
    let part-state = part-state.at(it.element.location())
    if (
      part-state == partTitle
        and counterInt.first() > 0
        and appendix-state == none
    ) {
      if it.level == 1 {
        v(0.5cm, weak: true)
